...
@router.post("/{rider_id}/await_submission")
async def await_submission(rider_id, rider_options: UserOptions, db: Session = Depends(get_db)):
    ...
    application_message = await bot.send_message(
        chat_id=rider_id,
        text=f"""
             К вашей поездке хочет присоединиться {rider.name}. {options_text} \n
             Связаться с пассажиром - 
             {f'@{rider.alias}' if rider.alias else f't.me/a/#{rider.alias}' if rider.alias else f't.me/a/#{rider.id}'}
         """,
        reply_markup=[
